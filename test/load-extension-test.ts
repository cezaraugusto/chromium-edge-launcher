...
import * as EdgeLauncher from '../src/edge-launcher.js';
...
describe('Load extension', () => {
  ...
  it('loadExtension via remote-debugging-pipe', async () => {
    ...
    const edgeFlags =
        EdgeLauncher.Launcher.defaultFlags().filter(flag => flag !== '--disable-extensions');
    edgeFlags.push('--remote-debugging-pipe', '--enable-unsafe-extension-debugging');
    const edgeInstance = await EdgeLauncher.launch({
      ignoreDefaultFlags: true, // Keep --disable-extensions out of flags
      edgeFlags,
      startingUrl: server.startUrl,
    });
    assert.equal(edgeInstance.port, 0, 'No --remote-debugging-port');

    const remoteDebuggingPipes = edgeInstance.remoteDebuggingPipes!;
